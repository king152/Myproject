...
# ZXXK_LOG = MySqlServer("10.1.5.70", 'zxxktest', '123456', 'Zxxk_Log')
...
def RecordDowninterfaceHits(InfoID, SchoolDown, PointDown, QCRdown, softpoint, Status, DownInfoID):
    ...
    # 回滚执行数据库语句
    updatasql = "UPDATE dbo.Cl_DownloadCountInfo SET ShoolUserDownNum='{}',PointUserDownNum='{}',QRCodeUserDownNum='{}',DownNum={} WHERE InfoID='{}'".format(
        SchoolDown, PointDown, QCRdown, DownNum, InfoID)

    ZXXK_LOG.insert_change_sql(updatasql)

    # 2.修改步骤表步骤状态及数据
    # 更新步骤状态
    sqlserver = MySqlServer("10.1.5.70", 'zxxktest', '123456', 'ZxxkDownload')  # 初始化数据库链接
...
def FeeBackLogForMonthly(Status, DownInfoID, Softid):
    ...
    Zxxk_Award = MySqlServer("10.1.5.70", 'zxxktest', '123456', 'ZxxkAward')
...
def HighWxtDownLoadStatisticsLog(Status, DownInfoID, InfoID):
    ...
    tempCount = downC[0][3]
    if tempCount >= 1:
        u_sql = "UPDATE dbo.Cl_WXTDownLoadStatistics SET TempCount='{}' WHERE InfoID='{}'".format(tempCount-1, InfoID)
        ZXXK_LOG.insert_change_sql(u_sql)
    else:
