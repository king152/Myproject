...
from download.common.performCase import runCase, runGetid, stepRunCase, getResult
...
from download.common.insertSql import initdownload, getSoftid_ordertime, requestdata
import threading
import json
import time
...
def case(request, project):
    ...
    paginator = JuncheePaginator(caseList, 15)
...
def details(request, project):
    ...
    case_list = CaseResult.objects.filter(executionUser=request.session.get("username"), project=project).order_by(
        '-executionTime')
    paginator = JuncheePaginator(case_list, 13)
    page = int(request.GET.get("page", 1))
    try:
        pages = paginator.page(page)
    except PageNotAnInteger:
        ...
        pages = paginator.page(paginator.num_pages)
    return render(request, 'channel/{}Detail.html'.format(project), {"pages": pages, "role": role})
...
def check(req):
    if req.method == "POST":
        get_caseId = req.POST.get("data[caseId]")
...
def startTestCase(request):
    if request.method == "POST":
        data = request.POST.get("data[caseId]")
        username = request.session.get("username")

        # 开启一个线程去执行用例
        threading.Thread(target=runCase, args=(data, username,)).start()
