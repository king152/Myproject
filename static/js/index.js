...
$(function () {
    $("#submit").click(function () {
        const $id = $('input[name="caseId"]').val().length;
        ...
        const $flag = $id !== 0 && $number !== 0 && $name !== 0 && $softtime !== 0 && $assercontent !== 0;
        if (!$flag) {
            alert("存在用例编号、用例名称、下载次数、上传时间、预期结果为空！")
        } else {
            $.ajax(
                {
                    type: "POST",
...
$(function () {
    $(".del").click(function () {
        const $id = $(this).parents("td").prevAll(":eq(8)").text();
